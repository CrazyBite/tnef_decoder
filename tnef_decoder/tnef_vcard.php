...
class TnefvCard
{
   ...
   function getName()
   {
      if ($this->name_is_unicode)
         return convert_unicode($this->name);

      return convert_windows_code_page($this->name, $this->getCodePage());
   }

   function getSurname()
   {
      if ($this->surname_is_unicode)
         return convert_unicode($this->surname);

      return convert_windows_code_page($this->surname, $this->getCodePage());
   }

   function getGivenName()
   {
      if ($this->given_name_is_unicode)
         return convert_unicode($this->given_name);

      return convert_windows_code_page($this->given_name, $this->getCodePage());
   }

   function getMiddleName()
   {
      if ($this->middle_name_is_unicode)
         return convert_unicode($this->middle_name);

      return convert_windows_code_page($this->middle_name, $this->getCodePage());
   }

   function getNickname()
   {
      if ($this->nickname_is_unicode)
         return convert_unicode($this->nickname);

      return convert_windows_code_page($this->nickname, $this->getCodePage());
   }

   function getCompany()
   {
      if ($this->company_is_unicode)
         return convert_unicode($this->company);

      return convert_windows_code_page($this->company, $this->getCodePage());
   }

//FIXME: what other attributes should be converted using convert_unicode() or convert_windows_code_page()??  Are the ones above appropriate?  getAddresses() probably should be converted, but it's an array of what, strings?
